...
|------------- | ------------- | -------------|
| [**createSideNote**](SubscriberNotesApi.md#createsidenoteoperation) | **POST** /v1/sides/{sideId}/notes | Create a note for a subscriber |
| [**deleteSideNote**](SubscriberNotesApi.md#deletesidenote) | **DELETE** /v1/sides/{sideId}/notes/{id} | Delete a note |
| [**updateSideNote**](SubscriberNotesApi.md#updatesidenoteoperation) | **PUT** /v1/sides/{sideId}/notes/{id} | Update a note |



## createSideNote

> SubResourceWriteResponse createSideNote(sideId, createSideNoteRequest, xCorrelationId, idempotencyKey)
...
import type { CreateSideNoteOperationRequest } from '';
...
async function example() {
  ...
  const api = new SubscriberNotesApi(config);
  ...
  } satisfies CreateSideNoteOperationRequest;

  try {
    const data = await api.createSideNote(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **createSideNoteRequest** | [CreateSideNoteRequest](CreateSideNoteRequest.md) |  | |
...
> deleteSideNote(sideId, id, xCorrelationId)
...
import type { DeleteSideNoteRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    // string | Optional correlation identifier for distributed tracing. If omitted, the server generates one. Echoed back in the response. (optional)
    xCorrelationId: xCorrelationId_example,
  } satisfies DeleteSideNoteRequest;

  try {
    const data = await api.deleteSideNote(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
> SubResourceWriteResponse updateSideNote(sideId, id, updateSideNoteRequest, xCorrelationId)
...
import type { UpdateSideNoteOperationRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    // string | Optional correlation identifier for distributed tracing. If omitted, the server generates one. Echoed back in the response. (optional)
    xCorrelationId: xCorrelationId_example,
  } satisfies UpdateSideNoteOperationRequest;

  try {
    const data = await api.updateSideNote(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **sideId** | `string` |  | [Defaults to `undefined`] |
| **id** | `string` |  | [Defaults to `undefined`] |
| **updateSideNoteRequest** | [UpdateSideNoteRequest](UpdateSideNoteRequest.md) |  | |
| **xCorrelationId** | `string` | Optional correlation identifier for distributed tracing. If omitted, the server generates one. Echoed back in the response. | [Optional] [Defaults to `undefined`] |
