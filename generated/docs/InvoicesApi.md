...
|------------- | ------------- | -------------|
| [**generateInvoices**](InvoicesApi.md#generateinvoices) | **POST** /v1/invoices/generate | Generate invoices for a period |
| [**previewInvoices**](InvoicesApi.md#previewinvoices) | **POST** /v1/invoices/preview | Preview invoices that would be generated for a period |



## generateInvoices

> InvoiceGenerateResponse generateInvoices(invoiceGenerateRequest, xCorrelationId, idempotencyKey)
...
import type { GenerateInvoicesRequest } from '';
...
async function example() {
  ...
  try {
    const data = await api.generateInvoices(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
|------------- | ------------- | ------------- | -------------|
| **invoiceGenerateRequest** | [InvoiceGenerateRequest](InvoiceGenerateRequest.md) |  | |
...
[**InvoiceGenerateResponse**](InvoiceGenerateResponse.md)
...
> InvoicePreviewResponse previewInvoices(invoicePreviewRequest, xCorrelationId, idempotencyKey)
...
import type { PreviewInvoicesRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    // string | UUID idempotency key. When present, the server guarantees at-most-once execution for the same key+endpoint within 24 hours. (optional)
    idempotencyKey: 38400000-8cf0-11bd-b23e-10b96e4ef00d,
  } satisfies PreviewInvoicesRequest;

  try {
    const data = await api.previewInvoices(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
[**InvoicePreviewResponse**](InvoicePreviewResponse.md)
