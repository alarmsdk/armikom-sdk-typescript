# SubscriberControlsApi

All URIs are relative to *http://localhost*

| Method | HTTP request | Description |
|------------- | ------------- | -------------|
| [**createSideControl**](SubscriberControlsApi.md#createsidecontroloperation) | **POST** /v1/sides/{sideId}/controls | Create a control record for a subscriber |
| [**deleteSideControl**](SubscriberControlsApi.md#deletesidecontrol) | **DELETE** /v1/sides/{sideId}/controls/{id} | Delete a control record |
| [**updateSideControl**](SubscriberControlsApi.md#updatesidecontroloperation) | **PUT** /v1/sides/{sideId}/controls/{id} | Update a control record |



## createSideControl

> SubResourceWriteResponse createSideControl(sideId, createSideControlRequest, xCorrelationId, idempotencyKey)
...
import type { CreateSideControlOperationRequest } from '';
...
async function example() {
  ...
  const api = new SubscriberControlsApi(config);
  ...
  } satisfies CreateSideControlOperationRequest;

  try {
    const data = await api.createSideControl(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **createSideControlRequest** | [CreateSideControlRequest](CreateSideControlRequest.md) |  | |
...
> deleteSideControl(sideId, id, xCorrelationId)
...
import type { DeleteSideControlRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    // string | Optional correlation identifier for distributed tracing. If omitted, the server generates one. Echoed back in the response. (optional)
    xCorrelationId: xCorrelationId_example,
  } satisfies DeleteSideControlRequest;

  try {
    const data = await api.deleteSideControl(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
> SubResourceWriteResponse updateSideControl(sideId, id, updateSideControlRequest, xCorrelationId)
...
import type { UpdateSideControlOperationRequest } from '';
...
async function example() {
  ...
  const body = {
    ...
    // string | Optional correlation identifier for distributed tracing. If omitted, the server generates one. Echoed back in the response. (optional)
    xCorrelationId: xCorrelationId_example,
  } satisfies UpdateSideControlOperationRequest;

  try {
    const data = await api.updateSideControl(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **sideId** | `string` |  | [Defaults to `undefined`] |
| **id** | `string` |  | [Defaults to `undefined`] |
| **updateSideControlRequest** | [UpdateSideControlRequest](UpdateSideControlRequest.md) |  | |
| **xCorrelationId** | `string` | Optional correlation identifier for distributed tracing. If omitted, the server generates one. Echoed back in the response. | [Optional] [Defaults to `undefined`] |
