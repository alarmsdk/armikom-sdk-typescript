...
|------------- | ------------- | -------------|
| [**exportSideStatusHistory**](SidesApi.md#exportsidestatushistory) | **POST** /v1/sides/status-history/export | Export the active/passive history of subscribers to CSV or XLSX |
...
## exportSideStatusHistory

> exportSideStatusHistory(sideStatusReportRequest, xCorrelationId, idempotencyKey)
...
import type { ExportSideStatusHistoryRequest } from '';
...
async function example() {
  ...
  } satisfies ExportSideStatusHistoryRequest;

  try {
    const data = await api.exportSideStatusHistory(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
|------------- | ------------- | ------------- | -------------|
| **sideStatusReportRequest** | [SideStatusReportRequest](SideStatusReportRequest.md) |  | |
